                        ...
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                ...
                                <label class="form-check-label" for="sendToAll">
                                    <strong>Отправить всем получателям</strong>
                                </label>
                            </div>
                            <small class="text-muted">
                                Отправит всем записям с заполненным email, исключая отписавшихся и невалидные адреса.
                                <br>Лимит: 400 писем в час (через Horizon).
                            </small>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    sendToAllCheckbox.addEventListener('change', function() {
        sendToAllEnabled = this.checked;

        if (sendToAllEnabled) {
            manualRecipients.style.display = 'none';
            selectedCount.textContent = 'All recipients';
            submitBtn.disabled = !templateSelect.value;
        } else {
            manualRecipients.style.display = 'block';
